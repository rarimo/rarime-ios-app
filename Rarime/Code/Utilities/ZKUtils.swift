...
class ZKUtils {
    ...
    public static func calcWtnsPhotoProof(
        _ privateInputsJson: Data
    ) throws -> Data {
#if targetEnvironment(simulator)
        return Data()
#else
        let descriptionFileData = Circuits.photoProofDat
        ...
        let result = witnesscalc_photoProof(
            (descriptionFileData as NSData).bytes, UInt(descriptionFileData.count),
            (privateInputsJson as NSData).bytes, UInt(privateInputsJson.count),
            wtnsBuffer, wtnsSize,
            errorBuffer, ERROR_SIZE
    ...
    public static func groth16PhotoProof(_ wtns: Data) throws -> (proof: Data, pubSignals: Data) {
        return try groth16Prover(Circuits.photoProofZkey, wtns)
    }
