...
class AppIconManager: ObservableObject {
    ...
    init() {
        self.appIcon = AppIcon(rawValue: UIApplication.shared.alternateIconName ?? "") ?? .black
    }
    ...
    func setAppIcon(_ icon: AppIcon) {
        ...
        UIApplication.shared.setAlternateIconName(icon.isDefault ? nil : icon.rawValue) { error in
            if let error = error {
